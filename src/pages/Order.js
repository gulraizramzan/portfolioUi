...
const handelOrder=async()=>{
    ...
    try {
      ...
      }else
      {
        ...
        const res=await ProductServices.post('/order/',{userId,shippingAddress:formData.shippingAddress,billingAddress:formData.billingAddress})
        setShowPopup(true)
      //   if(res)
...
<div class="flex min-h-screen items-center justify-center bg-gray-100">
  <div class="rounded-lg bg-gray-50 px-16 py-14">
    ...
    <button onClick={handelHome} class="mx-auto mt-10 block rounded-xl border-4 border-transparent bg-orange-400 px-6 py-3 text-center text-base font-medium text-orange-100 outline-8 hover:outline hover:duration-300">Go to Home</button>
  </div>
</div>
):  <div class="bg-gray-50 dark:bg-gray-800 w-full flex justify-between items-center md:items-start px-4 py-6 md:p-6 xl:p-8 flex-col mt-20">
<h3 class="text-xl dark:text-white font-semibold leading-5 text-gray-800">Address</h3>
<div class="flex flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0">
  <div class="flex flex-col justify-start items-start flex-shrink-0">
  </div>
  <div class="flex justify-between xl:h-full items-stretch  flex-col mt-6 md:mt-0">
    <div class="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row">
      <div class="flex justify-center  items-center md:items-start flex-col space-y-4 xl:mt-8">
      ...
      <div class="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4">
        ...
        <input 
          ...
          id="billingAddress" type="text"  className="block w-full px-4 py-2 mt-2 text-gray-700  border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring"/>
              ...
              <button onClick={handelOrder} class="mx-auto mt-10 block rounded-xl border-4 border-transparent bg-orange-400 px-6 py-3 text-center text-base font-medium text-orange-100 outline-8 hover:outline hover:duration-300">Place Order</button>
    </div>
  </div>
</div>
</div>}
