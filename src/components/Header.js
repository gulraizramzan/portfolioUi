...
const HeaderSecond=()=>{
  ...
  const togglePopup = () => {
    ...
    return(
      <>
<nav className=" border-gray-200 dark:bg-gray-900 bg-[url(https://images.pexels.com/photos/3621234/pexels-photo-3621234.jpeg)] bg-no-repeat">
  <div className="max-w-screen-xl flex flex-wrap  justify-between mx-auto p-4">
  <Link to="/home" className="flex items-center">
      <img src="https://flowbite.com/docs/images/logo.svg" className="h-8 mr-3" alt="Flowbite Logo" />
      <span className="text-2xl font-semibold whitespace-nowrap dark:text-white">GMART</span>
  </Link>
  <div className="md:order-2">
      <button onClick={togglePopup} type="button" className="flex text-sm bg-gray-800 rounded-full md:mr-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" id="user-menu-button" aria-expanded="false" data-dropdown-toggle="user-dropdown" data-dropdown-placement="bottom">
        <span className="sr-only">Open user menu</span>
        <image className="w-8 h-8 rounded-full" src="../images/banner.png" alt="user photo"/>
      </button>
    
     {
      showPopup &&(
        <div className="my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700 dark:divide-gray-600" id="user-dropdown">
        ...
        <ul className="py-2" aria-labelledby="user-menu-button">
          ...
          <li>
            <Link to="#" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Order</Link>
          </li>
          <li>
        <Link to="/cart" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Cart</Link>
          ...
          <li>
            <Link to="/login" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white" onClick={handelLogOut}>Sign out</Link>
          </li>
        </ul>
      </div>
      )
